...
button{border:1px solid var(--line);border-radius:7px;background:#1e293b;color:var(--text);padding:7px 12px;font:inherit;cursor:pointer}button:hover{background:#273449}button:focus-visible{outline:2px solid var(--accent);outline-offset:2px}.delete-job{padding:4px 8px;color:var(--danger)}.job-failed{background:var(--danger-bg);color:var(--danger)}.pagination{display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:14px}.pagination button:disabled{opacity:.45;cursor:default}
...
async function refresh(){
 const [s,r,l]=await Promise.all([fetch('/api/stats').then(x=>x.json()),fetch('/api/recent?page='+currentPage).then(x=>x.json()),fetch('/api/logs').then(x=>x.json())]);
