...
func (dashboard *DashboardServer) handleSelectTaskConfig(w http.ResponseWriter, request *http.Request) {
	...
	if err := dashboard.configs.SetVideoCRF(profile, selection.VideoCRF); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
const numberValue=(input,fallback)=>input.value.trim()===''?Number(input.defaultValue||fallback):Number(input.value);
